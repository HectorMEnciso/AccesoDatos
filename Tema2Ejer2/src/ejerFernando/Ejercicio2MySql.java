...
public class Ejercicio2MySql {
	public static void main(String[]args){
		...
		try
		{
			...
			Statement sen=null;
			ResultSet res=null;
			con=DriverManager.getConnection("jdbc:mysql://localhost/ejercicios", "root", "root");
		...
		while (emp!=0){
			...
			if (nemp!=0){
				...
				do {
					...
					dep=teclado.nextInt();
					ResultSet res4=sen.executeQuery("SELECT * FROM departamentos WHERE dept_no="+Integer.toString(dep));
					while (res4.next()){
						ndep++;
					}
					if (ndep==0){
...
private static int menu(Scanner teclado) {
	int op;
	System.out.println("MENU PARA INTERACTUAR CON LA BASE DE DATOS empresas.db EN MYSQL");
