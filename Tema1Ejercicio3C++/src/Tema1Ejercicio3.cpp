...
int main() {
	...
	do {
		...
		if (opcion == 1) {
			...
			salida << dato1.nombre << " " << dato1.apell1 << " " << dato1.apell2<< " " << dato1.telefono << endl;
			salida.close();
		} else if (opcion == 2) {
			ofstream salida2("c:\\archivos\\binario.txt",ios::out | ios::binary | ios::app); //ABRIMOS FLUJO ESCRITURA para añadir por el final
			cout << "Introduzca nombre: ";
			fflush(stdin);
			getline(cin, nombre);
			//cin >> nombre;
			...
			ifstream entrada2("c:\\archivos\\binario.txt",ios::in | ios::binary); //ABRIMOS FLUJO ENTRADA
			entrada2.read(reinterpret_cast<char*>(&dato4), sizeof(datos)); //LEEMOS UNO DEL ARCHIVO
			while (!entrada2.eof()) {
				cout << "NOMBRE: " << dato4.nombre << " 1º APELLIDO: "<< dato4.apell1 << " 2º APELLIDO: " << dato4.apell2<< " TELEFONO: " << dato4.telefono << endl; //MOSTRAMOS DATOS
