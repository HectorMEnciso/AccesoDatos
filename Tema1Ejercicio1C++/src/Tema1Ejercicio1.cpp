...
int main() {
	...
	for (int x = 1; x <= dim; x++) {
		alumno.clase = x;
		strcpy(alumno.apellido, nombres[rand() % 5].c_str());//Convertimos los string para la estrutura
	...
	while (!entrada.eof()) {//Mientras no sea final de fichero
	...
	cin >> clave;
	entrada1.seekg((clave - 1) * sizeof(mialumno), ios::beg); //NOS POSICIONAMOS EN EL BYTE CORRESPONDIENTE (NUMERO REGISTRO * TAMAÑO ESTRUCTURA) desde el principio
